...
files <- list.files(path = folder_path, pattern = "HumanAVG(ex)?_(A3|A5|AF|AL|MX|RI|SE)_strict\\.xlsx$", full.names = TRUE)
...
for (file in files) {
  ...
  event_type <- sub(".*HumanAVG(ex)?_(A3|A5|AF|AL|MX|RI|SE)_strict.*", "\\2", basename(file))
  ...
  results <- rbind(results, data.frame(File = basename(file), RowCount = row_count, Version = version, EventType = event_type, stringsAsFactors = FALSE))
}
