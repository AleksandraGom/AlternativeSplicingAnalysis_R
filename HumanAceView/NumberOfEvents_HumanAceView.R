...
files <- list.files(path = folder_path, pattern = "HumanAV(ex)?_(A3|A5|AF|AL|MX|RI|SE)_strict\\.xlsx$", full.names = TRUE)
...
for (file in files) {
  ...
  event_type <- sub(".*HumanAV(ex)?_(A3|A5|AF|AL|MX|RI|SE)_strict.*", "\\2", basename(file))
